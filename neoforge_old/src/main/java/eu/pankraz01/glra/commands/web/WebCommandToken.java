...
public final class WebCommandToken {
    ...
    private static int addToken(CommandContext<CommandSourceStack> ctx, String playerName) {
        ...
        try {
            ...
            var tokenComp = Component.literal(token).setStyle(Style.EMPTY
                    .withClickEvent(new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, token))
                    .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, tr(CREATED_COPY, "[copy]")))
            ...
            if (tokens.isEmpty()) {
                ...
                for (var t : tokens) {
                    ...
                    body.append(Component.literal("\n* "))
                            ...
                            .append(Component.literal(t.token())
                                    .withStyle(Style.EMPTY
                                            .withColor(ChatFormatting.GREEN)
                                            .withClickEvent(new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, t.token()))
                                            .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, tr(LIST_COPY_HINT, "Click to copy token")))))
            ...
            if (page > 1) {
                nav.append(Component.literal("\n<<")
                        .withStyle(Style.EMPTY.withColor(ChatFormatting.YELLOW).withBold(true)
                                .withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/gl web token list " + (page - 1)))
                                .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.literal("Zu Seite " + (page - 1))))));
            }
            if (hasNext) {
